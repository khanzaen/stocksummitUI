...
@section('content')
<div class="mb-8">
    <h2 class="font-outfit font-bold text-2xl text-white">Edit Timeline Event</h2>
    <p class="text-xs text-slate-400 mt-1">Modify milestone configuration, stage label, date range, or description</p>
</div>
...
<form action="{{ route('admin.timeline.update', $timeline) }}" method="POST" class="space-y-6 max-w-2xl">
    @csrf
    @method('PUT')
    ...
    <div class="glass-card border border-slate-800/80 p-6 sm:p-8 rounded-3xl shadow-xl space-y-6">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div class="space-y-2">
                ...
                <input type="text" name="stage" id="stage" value="{{ old('stage', $timeline->stage) }}" required
        ...
        <div class="space-y-2">
            ...
            <input type="text" name="date_range" id="date_range" value="{{ old('date_range', $timeline->date_range) }}" required
        ...
        <div class="space-y-2">
            ...
            <textarea name="description" id="description" rows="4" required
                      ...
                      placeholder="e.g. Assemble your teams, upload transcript files, and settle administration fees...">{{ old('description', $timeline->description) }}</textarea>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div class="flex items-center pt-8">
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="is_final" id="is_final" class="rounded border-slate-700 bg-slate-900 text-[#af8a3c] focus:ring-[#af8a3c]/30 w-5 h-5" {{ old('is_final', $timeline->is_final) ? 'checked' : '' }}>
